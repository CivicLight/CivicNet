...
* CivicNet Core MWEB uses tagged BLAKE3 hashes with one-byte domains (<code>'A'</code>, <code>'B'</code>, <code>'D'</code>, <code>'N'</code>, <code>'O'</code>, <code>'S'</code>, <code>'T'</code>, <code>'X'</code>, <code>'Y'</code>) and a multiplicative output-key tweak (<code>K<sub>o</sub> = B<sub>i</sub>⋅HASH32('O'||e)</code>).
...
* To be consistent with CivicNet Core, we recommend using the following derivation paths:
