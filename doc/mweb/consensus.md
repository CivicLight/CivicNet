...
* Kernels may include an optional pegin `amount`
  * Each pegin kernel must have a corresponding pegin output on the canonical CIVC side where the value is <code>v<sub>pegin</sub></code>, witness version is 9, and witness program is the kernel ID (<code>K<sub>ID</sub></code>).
