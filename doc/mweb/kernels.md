...
The peg-in amount shall be added to the total MWEB supply, and must have a corresponding pegin output on the canonical CIVC side.

#### Peg-Out

Kernels specifying the `PEGOUT_FEATURE_BIT` shall contain a vector of pegouts, each consisting of a (non-negative `var_int`) peg-out amount field and a valid 4-42 byte CIVC script.
