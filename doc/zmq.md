...
The CivicNet Core daemon can be configured to act as a trusted "border
router", implementing the civicnet wire protocol and relay, making
...
The ZeroMQ feature in CivicNet Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building civicnetd:
...
For instance:
    ...
    $ civicnetd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/civicnetd.tx.raw \
...
These options can also be provided in civicnet.conf.
...
From the perspective of civicnetd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into civicnetd directly. Furthermore, no information is
...
using. CivicNetd appends an up-counting sequence number to each
