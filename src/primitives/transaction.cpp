...
CMutableTransaction::CMutableTransaction(const CTransaction& tx) : vin(tx.vin), vout(tx.vout), nVersion(tx.nVersion), nLockTime(tx.nLockTime), mweb_tx(tx.mweb_tx), nTokenTxType(tx.nTokenTxType), tokenIssuePayload(tx.tokenIssuePayload), tokenConvertOutPayload(tx.tokenConvertOutPayload), m_hogEx(tx.m_hogEx) {}
...
CTransaction::CTransaction(const CMutableTransaction& tx) : vin(tx.vin), vout(tx.vout), nVersion(tx.nVersion), nLockTime(tx.nLockTime), mweb_tx(tx.mweb_tx), nTokenTxType(tx.nTokenTxType), tokenIssuePayload(tx.tokenIssuePayload), tokenConvertOutPayload(tx.tokenConvertOutPayload), tokenMintPayload(tx.tokenMintPayload), tokenVestingReleasePayload(tx.tokenVestingReleasePayload), tokenBurnPayload(tx.tokenBurnPayload), m_hogEx(tx.m_hogEx), hash{ComputeHash()}, m_witness_hash{ComputeWitnessHash()} {}
CTransaction::CTransaction(CMutableTransaction&& tx) : vin(std::move(tx.vin)), vout(std::move(tx.vout)), nVersion(tx.nVersion), nLockTime(tx.nLockTime), mweb_tx(tx.mweb_tx), nTokenTxType(tx.nTokenTxType), tokenIssuePayload(std::move(tx.tokenIssuePayload)), tokenConvertOutPayload(std::move(tx.tokenConvertOutPayload)), tokenMintPayload(std::move(tx.tokenMintPayload)), tokenVestingReleasePayload(std::move(tx.tokenVestingReleasePayload)), tokenBurnPayload(std::move(tx.tokenBurnPayload)), m_hogEx(tx.m_hogEx), hash{ComputeHash()}, m_witness_hash{ComputeWitnessHash()} {}
