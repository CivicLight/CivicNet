    ...
    try
    {
        for (; pc < pend; ++opcode_pos) {
            ...
            switch (opcode)
            {
                ...
                case OP_TOKEN_VESTING_LOCK: // repurposed OP_NOP5
                {
                    if (flags & SCRIPT_VERIFY_TOKEN_VESTING_LOCK) {
                        return set_error(serror, SCRIPT_ERR_TOKEN_VESTING_LOCKED);
                    }
                    if (flags & SCRIPT_VERIFY_DISCOURAGE_UPGRADABLE_NOPS)
                        return set_error(serror, SCRIPT_ERR_DISCOURAGE_UPGRADABLE_NOPS);
                }
                ...
                case OP_TOKEN_RESERVE: // repurposed OP_NOP4
                {
                    if (flags & SCRIPT_VERIFY_TOKEN_RESERVE) {
                        return set_error(serror, SCRIPT_ERR_TOKEN_RESERVE_LOCKED);
                    }
                    if (flags & SCRIPT_VERIFY_DISCOURAGE_UPGRADABLE_NOPS)
                        return set_error(serror, SCRIPT_ERR_DISCOURAGE_UPGRADABLE_NOPS);
                }
