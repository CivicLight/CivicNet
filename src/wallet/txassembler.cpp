    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional CIVC outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes CIVC and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::CIVC_TO_CIVC) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        // First try to construct a CIVC-to-CIVC transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::CIVC_ONLY;
        ...
        // If CIVC-to-CIVC fails, create a peg-out transaction
